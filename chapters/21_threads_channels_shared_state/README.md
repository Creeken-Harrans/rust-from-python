...
> 📚 **相关章节**：[05 所有权](../05_ownership_move_copy_clone/) | [06 借用](../06_references_borrowing_slices/) | [19 智能指针](../19_smart_pointers_box_rc_refcell/) | [22 异步编程](../22_async_await_tokio_intro/)
...
理解编译器**不能**做什么，比理解它能做什么更重要。Rust 在 safe 代码中消除了数据竞争——这是并发编程中最危险的一类 bug——但以下问题**不会被编译器检测或阻止**：

| 并发问题 | 编译器能检测？ | 说明 |
|---------|:---:|------|
| 数据竞争 | ✅ 编译期阻止 | Rust 的核心保证 |
| 死锁（Deadlock） | ❌ | 锁顺序是运行时行为，无法静态分析 |
| 活锁（Livelock） | ❌ | 线程在"尝试-放弃-重试"循环中空转 |
| 饥饿（Starvation） | ❌ | 某个线程长期得不到资源或 CPU 时间 |
| 锁粒度不当 | ❌ | 临界区太大或太小都是性能设计问题 |
| 业务逻辑错误 | ❌ | 并发逻辑的正确性仍需程序员判断 |
| 性能瓶颈 | ❌ | 锁争用、伪共享等问题需性能剖析 |

**死锁（Deadlock）**：两个线程互相等待对方持有的锁。本章[死锁](#死锁deadlock)一节展示了典型场景——当两个线程以不同顺序获取锁时，死锁就可能发生。虽然 `MutexGuard` 的 RAII 机制确保锁不会泄漏，但获取锁的顺序是编译器无法强制检查的运行时行为。

**活锁（Livelock）**：线程看似在"工作"但实际没有进展。例如两个线程使用 `try_lock()` 不断重试，每次恰好都发现对方持有自己想要的锁，于是同时释放已持有锁，再次重试——循环往复。

**饥饿（Starvation）**：某个线程长期得不到执行机会。在 `RwLock` 场景中，如果读者持续涌入，写者可能永远无法获取写锁（"写者饥饿"）。类似地，低优先级线程可能在高竞争下始终抢不到锁。

**锁粒度不当**：临界区太大（持有锁时做了不必要的工作，如文件 I/O）会降低并发度；临界区太小导致频繁加锁/解锁，带来不必要的系统调用开销。这些是**性能设计问题**，而非正确性问题。

**业务逻辑错误**：即使所有数据访问都被正确保护，程序的并发逻辑仍然可能出错。例如你正确地对 `AtomicI32` 执行了 `fetch_add`，但业务本意是 `fetch_sub`——编译器无法理解你的业务需求。

**性能瓶颈**：
- **锁争用**：多个线程频繁竞争同一个 `Mutex`，大量时间花在等待锁而非实际工作上
- **伪共享（False Sharing）**：两个线程操作不同变量，但这些变量位于同一个 CPU 缓存行上，导致缓存行在核心间"乒乓"传输
- **`Arc` 开销**：在热路径上频繁 `Arc::clone`，原子操作和内存屏障的开销可能成为瓶颈

这些问题需要结合 `perf`、火焰图、以及并发 profiler 等工具在生产环境中定位和优化。

### `Arc<Mutex<T>>` 是常见模式，但不是万能答案

`Arc<Mutex<T>>` 是 Rust 多线程共享可变数据的最常见组合，但**不是所有共享状态场景的默认选择**：

- **只读共享数据** → 直接用 `Arc<T>`，无需 Mutex 的开销
- **简单计数器/标志位** → `Arc<AtomicI32>` / `Arc<AtomicBool>`——原子操作比 Mutex 轻量一个数量级
- **读多写少** → `Arc<RwLock<T>>` 允许多个读者同时访问，提供更好的并发读取吞吐量
- **单向数据流** → `mpsc::channel` 消息传递，完全消除共享状态和锁
- **线程局部状态 + 最终归并** → 每个线程维护独立状态，最后通过 `join` 或 channel 汇总结果——rayon 的并行迭代器内部就采用了这个模式
- **单线程多所有权** → `Rc<RefCell<T>>` 比 `Arc<Mutex<T>>` 更轻量（无原子操作、无系统锁）

选择并发工具的黄金法则是：**根据数据访问模式选择工具，而非机械套用 `Arc<Mutex<T>>`**。Rust 的类型系统让你可以安全地尝试各种组合——编译器会阻止你用错。
...
C 和 C++ 的并发模型建立在操作系统原语之上。在 C 中，你需要直接使用 `pthread_create`、`pthread_mutex_lock`、`pthread_cond_wait` 等 POSIX 线程 API；C++11 之后，标准库提供了 `std::thread`、`std::mutex`、`std::lock_guard` 等 RAII 封装，但底层仍然是操作系统线程。

**核心问题：编译器不参与并发安全检查。** 在 C/C++ 中：

- 忘记加锁就访问共享数据？编译器**不会报错**——这是一个"合法"的程序，但运行时会表现为未定义行为
- 将栈上变量的指针传给另一个线程？悬垂指针导致的崩溃只能靠 ThreadSanitizer 等动态分析工具事后检测
- 在持锁期间意外返回或抛出异常？锁可能永远不会释放（尤其在 C 的 `pthread_mutex_lock`/`unlock` 手动配对场景）
- 数据竞争在 C/C++ 标准中是**未定义行为**，编译器有权做出任何假设——包括优化掉你认为是"安全检查"的代码

几十年来，C/C++ 社区积累了大量的并发最佳实践：RAII 风格的锁守卫、锁排序（lock ordering）、不可变共享数据等模式。但**语言本身不提供编译期保证**——安全性完全依赖程序员的纪律和代码审查。

### Python 的 GIL 与并发模型

CPython（最广泛使用的 Python 实现）使用**全局解释器锁（GIL）**来保护解释器内部状态：

- **同一时刻只有一个线程执行 Python 字节码**。即使你在 8 核 CPU 上启动了 8 个线程，同一瞬间也只有一个核在执行 Python 代码
- CPU 密集型任务**无法通过多线程加速**——GIL 使多线程变成了事实上的串行执行，甚至可能因锁争用和上下文切换而比单线程更慢
- Python 多线程主要用于**I/O 密集型任务**（网络请求、文件读写、数据库查询），因为线程在等待 I/O 时会主动释放 GIL
- 要真正利用多核 CPU 的并行计算能力，Python 程序员有两个选择：
  - **`multiprocessing` 模块**：启动多个独立进程，每个进程有自己的 GIL 和地址空间，通过 pickle 序列化或共享内存通信
  - **编写 C 扩展**：C 扩展可以在执行计算密集型任务期间通过 `Py_BEGIN_ALLOW_THREADS`/`Py_END_ALLOW_THREADS` 宏释放 GIL，实现真正的多线程并行
- GIL 在字节码层面提供了粗粒度的保护，但**不保证操作的原子性**：`x += 1` 在 Python 字节码层面被编译为多条指令，两个线程可能交替执行这些指令，导致"丢失更新"

**总结**：GIL 简化了 CPython 的实现，也是 Python 单线程性能优化的基础，但它从根本上限制了多线程并行计算的能力。

### Rust 的线程安全不等于自动并行

Rust 的"无畏并发"（Fearless Concurrency）常被误解为"Rust 会自动并行化代码"。这是一个重要的澄清：

- **Rust 不自动并行化你的代码**。你需要显式地调用 `thread::spawn`、`Arc::clone`、`mutex.lock()`
- Rust 不提供自动负载均衡或工作窃取调度器（这些由 rayon、tokio 等第三方库提供）
- Rust 的保证是**防御性的**：确保并发代码不会因为数据竞争而崩溃或产生未定义行为
- 你仍然需要理解并发的核心概念（锁、原子操作、内存序），才能写出**正确且高效**的多线程程序

Rust 给你的是一个**类型安全的并发工具箱**，而不是自动并行的魔法棒。

### Send 与 Sync：类型级别的线程安全合约

Rust 通过两个标记 trait（marker trait）——无任何方法，仅作为编译期标记——将线程安全规则编码进类型系统。

**`Send`——"这个值可以安全地交给另一个线程"**

当 `T: Send` 时，意味着 `T` 的所有权可以跨线程转移。编译器通过这个 trait 回答：如果把这块内存的所有权移交给另一个线程，是否安全？

- 几乎所有 Rust 基本类型都是 `Send`
- `Rc<T>` **不是** `Send`：因为它的引用计数操作使用非原子的 `Cell<usize>`——两个线程同时 clone/drop 会导致计数错乱
- 编译器自动推导：结构体是 `Send` 当且仅当其所有字段都是 `Send`

**`Sync`——"多个线程可以同时持有这个值的共享引用"**

当 `T: Sync` 时，意味着 `&T` 可以安全地被多个线程同时持有。等价地说：`T: Sync` 当且仅当 `&T: Send`。

- `RefCell<T>` **不是** `Sync`：它的运行时借用计数器不是原子操作——多线程同时 `borrow()` 会导致计数错乱，破坏"运行时单个可变借用"的保证
- `Mutex<T>` 是 `Sync`（当 `T: Send` 时）：通过系统级锁将运行时的并发访问串行化
- 基本类型都是 `Sync`：`i32`、`bool`、`&str` 等——多个线程同时读取不可变数据没有任何风险

这两个 trait 是 Rust 编译期线程安全保障的基石。当你调用 `thread::spawn` 时，编译器会静默检查闭包中所有捕获的值是否满足 `Send`；当你共享 `Arc<T>` 时，编译器确保 `T: Send + Sync`。

### 共享所有权类型对照表

下表总结了 Rust 中涉及"所有权共享"和"内部可变性"的核心类型在多线程场景中的能力：

| 类型 | 共享所有权 | 允许跨线程 | 可变访问方式 |
|------|:---:|:---:|------|
| `Rc<T>` | ✅ | ❌ | 需要额外机制 |
| `Arc<T>` | ✅ | ✅ | 需要额外同步机制 |
| `RefCell<T>` | 不负责共享所有权 | ❌ | 运行时借用检查 |
| `Mutex<T>` | 不负责共享所有权 | 可用于线程同步 | 加锁后访问 |
| `Arc<Mutex<T>>` | ✅ | ✅ | 加锁后修改 |

**解读**：

- `Rc<T>` 和 `Arc<T>` 是**所有权共享**工具——回答"多个持有者共同拥有数据"的问题，但不解决"如何修改数据"
- `RefCell<T>` 和 `Mutex<T>` 是**内部可变性**工具——回答"如何在不改变外部不可变性的前提下修改数据"，但不解决"谁拥有数据"
- `Arc<Mutex<T>>` 将两者组合，同时解决所有权共享和线程安全修改两个问题——这正是它成为多线程编程中最常见模式的原因
- 但 `Arc<Mutex<T>>` **不是银弹**：对于只读数据，`Arc<T>` 就够了（无需锁开销）；对于简单计数器，`Arc<AtomicI32>` 比 `Arc<Mutex<i32>>` 高效得多；对于读多写少的场景，`Arc<RwLock<T>>` 提供更好的并发读取性能
