...
> 以下问题帮助你思考 C++/Python 中的共享与复制模式如何用 Rust 的 ownership 重新建模。

### 问题 1：C++ 中 shared_ptr 共享数据的方式，在 Rust 中应该怎么重新建模？

C++ 中你可能会用 `std::shared_ptr<T>` 让多个对象共享同一份数据的所有权，引用计数归零时自动释放。如果你将同样的场景迁移到 Rust，`Rc<T>` 和 `Arc<T>` 分别对应什么使用条件？但更重要的是：在很多场景下，C++ 程序员用 `shared_ptr` 仅仅是因为不需要"想清楚谁拥有数据"。Rust 的所有权系统要求你在设计阶段就明确谁是数据的唯一所有者——这种思维转变会怎样影响你的 API 设计？

**提示**：先尝试用唯一所有者和借用（&T）来满足需求，只有在确实需要共享所有权时才引入 Rc/Arc。

### 问题 2：遇到所有权错误就用 clone() 解决的做法有什么问题？

初学 Rust 时，编译器的 move error 让人沮丧，于是很多学习者养成了一个习惯：哪里报错就加 `.clone()`。这种做法在 C++ 中也很常见（`std::string` 的各种拷贝）。请思考：过度使用 clone 会带来什么问题？从性能、语义和借用检查器的设计意图三个角度分析。如果一个函数的参数本来可以声明为 `&T` 但你选择了按值传递 + clone 调用者数据，这说明你在设计上错过了什么？

**提示**：clone 不是免费的——每次 clone 都意味着堆分配和数据的深拷贝；借用检查器"难"正是因为它在引导你找到无需拷贝的正确设计。
