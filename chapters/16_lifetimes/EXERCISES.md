...
> 以下问题帮助你思考 C/C++ 中的悬垂指针和 use-after-free 问题如何被 Rust 的生命周期机制在编译期消灭。

### 问题 1：在 C/C++ 中哪些场景会出现"use-after-free"，Rust 如何通过生命周期防止？

回顾你在 C/C++ 中遇到过或听说过的内存 bug：返回局部变量的指针、迭代器失效、realloc 后继续使用旧指针、在 vector 扩容后持有旧元素的引用……请逐一思考这些场景在 Rust 中由什么机制阻止。尤其是"返回局部变量的引用"——C/C++ 编译器可能只给一个 warning，Rust 的编译器如何处理？生命周期标注 `'a` 在跨函数的引用传递中扮演了什么角色？

**提示**：Rust 的生命周期是类型的组成部分——当你写 `fn foo<'a>(x: &'a str) -> &'a str` 时，你在告诉编译器"返回值的引用不会比参数 x 活得长"。

### 问题 2：什么时候不应该返回引用，而应该返回拥有所有权的值？

有一个常见的 C++ 设计陷阱：一个类的方法返回 `const string&` 指向其内部成员，调用方把这个引用存起来，然后原始对象被销毁了——use-after-free。在 Rust 中，生命周期系统会阻止你写出"返回指向局部数据的引用"的代码。但这也引出一个设计问题：什么时候你**应该**返回 `String`（拥有所有权）而不是 `&str`（借用）？如果返回引用会逼迫你给结构体和方法加上复杂的生命周期标注，这是否是"重新审视设计"的信号？

**提示**：当你发现生命周期标注变得复杂（多个 `'a`、`'a: 'b` 约束），往往意味着你的数据结构过于耦合；考虑返回拥有所有权的值或用 `Arc` 来简化生命周期模型。
